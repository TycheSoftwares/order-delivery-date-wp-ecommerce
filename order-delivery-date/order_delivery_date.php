<?php
/*
Plugin Name: Order Delivery Date for WP eCommerce
...
register_uninstall_hook( __FILE__, 'wpefield_deactivate' );
...
if ( !class_exists( 'order_delivery_date_wp_ecommerce_lite' ) ) {
    class order_delivery_date_wp_ecommerce_lite {
    
        public function __construct() {
            add_action( 'wpsc_before_form_of_shopping_cart', array( &$this, 'wpefield_delivery_date' ) );
            register_activation_hook( __FILE__, array( &$this, 'wpefield_activate' ) );
            //adds the files in the head of the admin settings form page
            add_action( 'admin_head', array( &$this, 'order_delivery_date_adminside_head' ) );
            //frontside scripts
            add_action( 'wp_enqueue_scripts', array( &$this, 'order_delivery_date_front_scripts' ) );
            add_action( 'admin_menu', array( &$this, 'order_delivery_date_menu' ) );
            add_action( 'admin_init', array( &$this, 'order_delivery_date_save' ) );
        }
        ...
        function wpefield_delivery_date() {
        	...
        	if( $results != null ) {
        		...
        		$query = "INSERT INTO $wpefield__TABLE (`id`, `name`, `type`, `mandatory`, `display_log`, `default`, `active`, `checkout_order`, `unique_name`, `options`, `checkout_set`) VALUES ('', 'Delivery Date', 'text', '0', '0', '0', '1', $max_count, 'e_deliverydate', '', '0');";
        		$wpdb->query( $query );
        ...
        function order_delivery_date_menu() {
            add_menu_page( 'Order Delivery Date', 'Order Delivery Date', 'administrator', 'order_delivery_date', array( &$this, 'order_delivery_date_settings' ) );
        }
        ...
        function order_delivery_date_save() {
            if( isset( $_POST[ 'save' ] ) ) {
...
$order_delivery_date_wp_ecommerce_lite = new order_delivery_date_wp_ecommerce_lite();
?>